...
class Mlp(nn.Cell):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=mint.nn.GELU, drop=0.):
        super().__init__()
    ...
    def construct(self, x, H, W):
        B, _, C = x.shape
        x = mint.permute(x, (0, 2, 1)).reshape(B, C, H, W)
        x = self.conv1(x)
        x = self.drop(x)
        x = self.proj(x) + x
        x = self.proj_act(x)
        x = self.proj_bn(x)
        x = self.conv2(x)
        x = mint.permute(x.reshape(B, C, -1), (0, 2, 1))
        x = self.drop(x)
        return x
...
class Attention(nn.Cell):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None,
        ...
        self.q = mint.nn.Linear(dim, self.qk_dim, bias=qkv_bias)
        self.k = mint.nn.Linear(dim, self.qk_dim, bias=qkv_bias)
    ...
    def construct(self, x, H, W, relative_pos):
        ...
        if self.sr_ratio > 1:
            x_ = mint.permute(x, (0, 2, 1)).reshape(B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1)
            x_ = mint.permute(x_, (0, 2, 1))
            k = self.k(x_).reshape(B, -1, self.num_heads,
                                   self.qk_dim // self.num_heads)
            k = mint.permute(k, (0, 2, 1, 3))
        ...
        attn = mint.matmul(q, mint.permute(
            k, (0, 1, 3, 2))) * self.scale + relative_pos

        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        x = mint.matmul(attn, v)
        x = mint.permute(x, (0, 2, 1, 3)).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Cell):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=mint.nn.GELU, norm_layer=mint.nn.LayerNorm, qk_ratio=1, sr_ratio=1):
        super().__init__()
    ...
    def construct(self, x, H, W, relative_pos):
        B, _, C = x.shape
        cnn_feat = mint.permute(x, (0, 2, 1)).reshape(B, C, H, W)
        x = self.proj(cnn_feat) + cnn_feat
        x = mint.permute(x.reshape(B, C, H * W), (0, 2, 1))
...
class PatchEmbed(nn.Cell):
    ...
    def construct(self, x):
        ...
        x = self.proj(x)
        _B, _C, _H, _W = x.shape
        x = mint.permute(x.reshape(_B, _C, _H * _W), (0, 2, 1))
        x = self.norm(x)
...
class CMT(nn.Cell):
    def __init__(
        ...
        self.stem_conv2 = mint.nn.Conv2d(
    ...
    def forward_features(self, x):
        ...
        x = mint.permute(x.reshape(B, H, W, -1), (0, 3, 1, 2))
        ...
        x = self._fc(mint.permute(x, (0, 2, 1)).reshape(B, C, H, W))
        x = self._bn(x)
        x = swish(x)
